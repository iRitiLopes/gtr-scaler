...
class FretboardProjector:
    """Encapsulates fretboard projection logic."""

    def __init__(self, notes: NoteService) -> None:
        self._notes = notes
        self._open_semitones: tuple[int, ...] = tuple(
            notes.to_semitone(n) for n in ("E", "A", "D", "G", "B", "E")
        )

    @property
    def open_semitones(self) -> tuple[int, ...]:
        """Read-only access to the open-string semitone values."""
        return self._open_semitones

    def project(
        self, root: str, scale: Scale, fret_start: int = 0, fret_end: int = 12
    ) -> list[FretCell]:
        """Return all fretboard positions that belong to the scale within [fret_start, fret_end]."""
        root_semitone = self._notes.to_semitone(root)
        ...
        cells: list[FretCell] = []
        for string_idx, open_semitone in enumerate(self._open_semitones):
    ...
    def _first_note_gte(self, open_semitone: int, target_pitch: int, min_fret: int) -> int:
    ...
    def project_n_notes(
        self, root: str, scale: Scale, notes_per_string: int, fret_start: int = 0
        ...
        for string_num, (string_idx, open_semitone) in enumerate(
            zip(range(len(self._open_semitones)), self._open_semitones)
        ):
            if string_num == 0:
                start_fret = fret_start
            else:
                start_fret = self._first_note_gte(
    ...
    def degree_fret_start(self, root: str, scale: Scale, degree: int) -> int:
        ...
        if degree < 1 or degree > len(scale.intervals):
            ...
            raise ValueError(f"Degree {degree} out of range for a {n}-note scale")
        root_semitone = self._notes.to_semitone(root)
        degree_semitone = (root_semitone + scale.semitones[degree - 1]) % 12
        open_e = self._open_semitones[0]  # E = 4
    ...
    def degree_fret_start_with_shift(
        ...
        of the neck.
        """
        base_fret = self.degree_fret_start(root, scale, degree)
        if base_fret == 0:
            cells, _ = self.project_n_notes(root, scale, notes_per_string, base_fret)
            for i in range(1, 6):
